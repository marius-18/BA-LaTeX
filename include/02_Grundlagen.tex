...
\label{def:common_disjoint}
	...
	\red{Weiterhin gilt $N_{c}(u,v) \cap N_{d}(u,v) = \emptyset$ und $N_{c}(u,v) \cup N_{d}(u,v) = N(u) \cup N(v)$.
	Jeder Knoten $x\in N(u) \cup N(v)$ aus den beiden Nachbarschaften liegt also entweder
...
die Knoten aus einer der beiden Bipartitionsklassen \red{die größere? oder egal? } \red{\Large soll ich der
besser einen namen geben? welchen?}
...
disjunkten Nachbarschaft tauschen.\red{fehlt hier noch was?}
...
\\
Ein \fett{Global \ct{}} besteht aus mehreren Curveball-Tauschen, wobei möglichst jeder Knoten aus der \red{Partitionsklasse}
...
Hierbei können wir ausnutzen, dass der Eingabegraph bipartit ist. Da es innerhalb des \red{\partvek{s}}
...
also parallel, ausführen. \red{Diese Parallelität führt natürlich zu einem Laufzeitvorteil.}
\\
Der hauptsächliche Unterschied zwischen \gc{} auf allgemeinen und bipartiten Graphen liegt also darin, 
dass die einzelnen \ct{e} sich gegenseitig nicht beeinflussen und daher parallel behandelt werden können.
\\
\\
...
aperiodisch, irreduzibel und symmetrisch ist \cite{...?} \red{welche quelle?}. Ebenso ist die Markov-Kette endlich, da die Anzahl an
