...
$V_1 = \{v_1,v_2,v_3\}$ und $V_2 = \{v_{4},v_5,v_6\}$. Man sieht deutlich, dass alle Kanten die Partitionsklassen
...
\nk{} \cite{nk} ist ein Open-Source Projekt, dass es \glqq zum Ziel hat, Werkzeuge für die
...
sind um sie auf \red{Multicore/Mehrkern} Architekturen zu nutzen.
\grqq\footnote{\red{Übersetzt aus \cite{nk_page}}}
\nk{} ist ein Python Modul, wobei jedoch die Algorithmen aus Gründen
der Performance in \cpp programmiert sind. Für die Parallelisierungen wird OpenMP verwendet.
\\

Mit \nk{} lassen sich in einfacher Art und Weise Graphen erstellen  und verschiedene
Algorithmen auf ihnen ausführen. So können sich beispielsweise Standardalgortihmen
wie Breitensuche, Tiefensuche oder Dijkstra's Algorithmus ausgeführt werden. 
Zum Randomisieren von Graphen sind in \nk{} --- die in der Einleitung schon genannten 
...
dass dadurch Multikanten entstehen. Dies geschieht
...
Partitonsklasse geändert. Die Partitionsklasse auf der die \ct{e} ausgeführt werden, wird als aktiv 
bezeichnet. 
\\
...
\gc{}-Tausch ineinander überführbar sind. Diese Markov-Kette aperiodisch, irreduzibel und symmetrisch \cite{penschuck2020recent}.
 Ebenso ist die Markov-Kette endlich, da die Anzahl an
Graphen mit gegebener Gradsequenz beschränkt ist. \red{In \cite{markov_converge}}
wird bewiesen, dass solche
Markov-Ketten nach hinreichend vielen Schritten zu einer uniformen Verteilung auf den Zuständen konvergiert.
...
die Knoten aus einer der beiden Partitionsklassen 
in einem Array speichern.
Diese
wird die aktive Partition sein, auf der die \ct{e} ausgeführt werden.
...
Das Array, in welchem die Knoten gespeichert werden, wird im Folgenden als \partvek{} bezeichnet. 
Für jeden Knoten des \partvek{s} wird jeweils wieder ein Array erstellt, 
in dem alle adjazenten Knoten gespeichert werden.
\\
    ...
    \foreach \x in {0,1,2,3}
		{
			...
			\fill [ fill =lightgray, draw =black ]  (\x ,-1.5) rectangle (\x+1 ,-0.5) ;
		};
